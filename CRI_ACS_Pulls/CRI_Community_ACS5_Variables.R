...
get_acs_vars <- function(year) {
  load_variables(year, "acs5/subject")
}

cri_get_acs <- function(geography, year) {
  get_acs(
    geography = geography,
...
comm_ntxcountiesC <- get_acs(
  geography = "county", variables = comm_variables,
