...
econ_Svariables <- c(
  ...
  econ_popov16 = "S0101_C01_025"
        ...
        #econ_juvunem: currently unable to find in ACS census, will calculate pop estimate later.
...
econ_cityofdallS <- get_acs(geography = "place", variables = econ_Svariables,
                           state = "TX", year = 2018, survey = "acs5", output = "wide") %>%
  filter(GEOID == 4819000) %>%
  mutate(
    GEOTYPE = "PLACE")

econ_cityofdallB <- get_acs(geography = "place", variables = econ_medinc,
        state = "TX", year = 2018, survey = "acs5", output = "wide") %>%  
  filter(GEOID == 4819000) %>%
  mutate(
    GEOTYPE = "PLACE")
...
econ_dallcountyS <- get_acs(geography = "tract", variables = econ_Svariables,
                           state = "TX", county = "dallas", year = 2018, survey = "acs5", output = "wide") %>%
  mutate(GEOTYPE = "TRACT")

econ_dallcountyB <- get_acs(geography = "tract", variables = econ_medinc,
                            state = "TX", county = "dallas", year = 2018, survey = "acs5", output = "wide") %>%  
  mutate(
    GEOTYPE = "TRACT")
...
econ_ntxcountiesTs <- get_acs(geography = "tract", variables = econ_Svariables,
                             state = "TX", county = counties, year = 2018, survey = "acs5", output = "wide") %>%
  mutate(GEOTYPE = "TRACT")

econ_ntxcountiesCs <- get_acs(geography = "county", variables = econ_Svariables,
                             state = "TX", county = counties, year = 2018, survey = "acs5", output = "wide") %>%
...
econ_ntxcountiesTb <- get_acs(geography = "tract", variables = econ_medinc,
                             state = "TX", county = counties, year = 2018, survey = "acs5", output = "wide") %>%
  mutate(GEOTYPE = "TRACT")

econ_ntxcountiesCb <- get_acs(geography = "county", variables = econ_medinc,
                             state = "TX", county = counties, year = 2018, survey = "acs5", output = "wide") %>%
  mutate(GEOTYPE = "COUNTY")
...
econ_dfwS <- get_acs(geography = "metropolitan statistical area/micropolitan statistical area", variables = econ_Svariables,
                     year = 2018, survey = "acs5", output = "wide") %>%
  filter(GEOID == 19100) %>%
  mutate(GEOTYPE = "MSA")

econ_dfwB <- get_acs(geography = "metropolitan statistical area/micropolitan statistical area", variables = econ_medinc,
                     year = 2018, survey = "acs5", output = "wide") %>%  
  filter(GEOID == 19100) %>%
  mutate(
    GEOTYPE = "MSA")
...
#merge datasets
econ_geo <- full_join(full_join(full_join(econ_cityofdall, econ_dallcounty), econ_dfw), econ_ntxcounties)
...
econ_geoFINAL <- econ_geo %>%
  select(-(econ_juvpop1E:econ_juvpop2M), -(econ_juvunemM1E:econ_juvunemF3M)) %>%
  select("GEOID", "NAME", "GEOTYPE", sort(names(.)))


#Export to .csv
export(econ_geoFINAL, "PASTE EXPORT DIRECTORY HERE")
