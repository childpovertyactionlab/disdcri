...
health.r <- health %>%
  ...
  select("TEA", "SLN", "AreaSqMi", "SCHOOLNAME", everything(.))
rm(health.j)
...
health.d <- cbind(var, stat.desc(health.i))
rm(var)
...
health.n <- as_tibble(do.call(rbind, lapply(health.i, function(x) shapiro.test(x)[c("statistic", "p.value")])), rownames = "var") %>%
  filter(p.value < 0.05)
health.n
