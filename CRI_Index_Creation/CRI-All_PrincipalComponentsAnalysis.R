...
cri <- left_join(left_join(left_join(left_join(community, economics), education), family), health) %>%
...
#export tables
export(criI.cos, "SET AS EXPORT DIRECTORY/CRI - PCA Indiviual Cos.csv")
export(criV.cos, "SET AS EXPORT DIRECTORY/CRI - PCA Variable Cos.csv")
export(cri_fin, "SET AS EXPORT DIRECTORY/CRI Sub-Indices Weighted.csv")
export(cri_scale, "SET AS EXPORT DIRECTORY/CRI Sub-Indices Weighted and Scaled.csv")
